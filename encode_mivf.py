...
class EncodeSettings:
    ...
    audio_mux_engine: str = "python"
    video_stream: int = 0  # relative video-stream index; 0 preserves legacy behavior
    audio_stream: int = 0  # relative audio-stream index; 0 preserves legacy behavior
    subtitle_stream: int | None = None  # relative subtitle-stream index; None = no sidecar authored (legacy)
    subtitle_edition: int = 0  # player sidecar slot 0-3 (source/main.c hfix58s_subtitles_load_for_video); only meaningful with subtitle_stream set
...
def make_subtitle_sidecar_path(mivf_path: Path, edition: int = 0) -> Path:
    """Mirrors source/main.c's MIVF_SubtitlesMakeSidecarPath + the
    subtitle_track_index convention in hfix58s_subtitles_load_for_video():
    edition 0 -> "<basename>.srt", edition 1-3 -> "<basename>.<N>.srt",
    same directory as the .mivf. Never a free-form path -- deterministic
    from the real output path, so an authored sidecar is always exactly
    where the player will look for it."""
    stem = mivf_path.stem
    if edition == 0:
        return mivf_path.with_name(f"{stem}.srt")
    return mivf_path.with_name(f"{stem}.{edition}.srt")


def extract_subtitle_sidecar(source: Path, mivf_path: Path, subtitle_stream: int, edition: int) -> Path:
    """Extracts one subtitle stream to the player's SRT sidecar convention.

    MediaProbe's "text" classification is necessary but not sufficient for
    success (source-audited this phase: TTML is classified "text" but this
    project's bundled ffmpeg cannot mux it even into MKV) -- so this always
    attempts the real subprocess and reports its real failure rather than
    trusting the codec classification alone. Writes to a temp path first and
    only replaces the final sidecar atomically on confirmed success, so an
    interrupted extraction can never leave a truncated sidecar at the path
    the player will actually read."""
    target = make_subtitle_sidecar_path(mivf_path, edition)
    tmp = target.with_suffix(target.suffix + ".tmp")
    ffmpeg = mivf_ffmpeg_path()
    cmd = [
        ffmpeg, "-y", "-hide_banner", "-loglevel", "error",
        "-i", str(source),
        "-map", f"0:s:{subtitle_stream}",
        "-c:s", "srt",
        "-f", "srt",  # the temp path's ".tmp" suffix defeats FFmpeg's own
                       # extension-based format auto-detection; force it
                       # explicitly rather than relying on the filename
        str(tmp),
    ]
    try:
        ...
        if tmp.exists():
            tmp.unlink()
        raise SystemExit(f"subtitle extraction failed to launch FFmpeg: {exc}")
    ...
    if result.returncode != 0 or not tmp.exists() or tmp.stat().st_size == 0:
        ...
        raise SystemExit(
            f"subtitle extraction failed for stream 0:s:{subtitle_stream} "
...
def e0_settings_fingerprint(input_path: Path, settings: "EncodeSettings") -> dict:
    ...
    return {
        ...
        "video_stream":settings.video_stream,"audio_stream":settings.audio_stream,
        "subtitle_stream":settings.subtitle_stream,"subtitle_edition":settings.subtitle_edition,
...
def e0_validate_preflight(input_path: Path, settings: "EncodeSettings") -> int:
    ...
    if settings.video_stream < 0 or settings.audio_stream < 0:
        raise SystemExit("--video-stream and --audio-stream must be >= 0")
    if settings.subtitle_stream is not None and settings.subtitle_stream < 0:
        raise SystemExit("--subtitle-stream must be >= 0")
    if not (0 <= settings.subtitle_edition <= 3):
        raise SystemExit("--subtitle-edition must be between 0 and 3 (matches the player's 4 sidecar slots)")
    if settings.subtitle_edition != 0 and settings.subtitle_stream is None:
        raise SystemExit("--subtitle-edition requires --subtitle-stream")
...
def start_ffmpeg_audio_pipe(source: Path, rate: int, channels: int, audio_stream: int = 0) -> subprocess.Popen[bytes]:
    ffmpeg = mivf_ffmpeg_path()
    cmd = [
        ffmpeg,
        "-hide_banner",
...
def find_ia4m_stream_helper() -> Path | None:
    here = Path(__file__).resolve().parent
    candidates = [
        resource_dir() / "mivf_ia4m_stream.exe",
        resource_dir() / "tools" / "mivf_ia4m_stream.exe",
        here / "tools" / "mivf_ia4m_stream.exe",
        here / "mivf_ia4m_stream.exe",
        resource_dir() / "mivf_ia4m_stream",
        resource_dir() / "tools" / "mivf_ia4m_stream",
        here / "tools" / "mivf_ia4m_stream",
    ]
    for candidate in candidates:
        if candidate.exists():
            return candidate
...
def start_native_ia4m_stream(samples_per_frame: int, start_frame: int = 0) -> subprocess.Popen[bytes]:
    exe = find_ia4m_stream_helper()
    if exe is None:
        raise SystemExit(
            "--audio-mux-engine native requested, but mivf_ia4m_stream(.exe) was not found.\n"
            "Build it with:\n"
            "  gcc -O2 -std=c99 -Wall -Wextra -o tools/mivf_ia4m_stream.exe tools/mivf_ia4m_stream.c"
        )
    return subprocess.Popen([str(exe), str(samples_per_frame), str(start_frame)], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, bufsize=0)


def read_native_ia4m_body(proc: subprocess.Popen[bytes], pcm: bytes, frame_no: int) -> bytes:
    if proc.stdin is None or proc.stdout is None:
        raise SystemExit("native IA4M helper pipe is not open")
    proc.stdin.write(pcm)
    proc.stdin.flush()
    hdr = read_exact_or_eof(proc.stdout, 4)
    if len(hdr) != 4:
        raise SystemExit(f"native IA4M helper produced a short length header at frame {frame_no}")
    body_size = le32(hdr, 0)
    if body_size < 20 or body_size > 65536:
        raise SystemExit(f"native IA4M helper produced invalid body size {body_size} at frame {frame_no}")
    body = read_exact_or_eof(proc.stdout, body_size)
    if len(body) != body_size:
        raise SystemExit(f"native IA4M helper produced a short body at frame {frame_no}")
    if body[:4] != b"IA4M":
        raise SystemExit(
            f"native IA4M helper produced a desynchronized packet at frame {frame_no}: "
            f"size={body_size} magic={body[:4]!r}"
        )
    return body
...
def mux_audio_into_mivf(video_mivf: Path, audio_src: Path, out_path: Path, rate: int, channels: int, workdir: Path, audio_codec: str = "ia4m", audio_offset_ms: int = 0, audio_mux_engine: str = "python", audio_stream: int = 0) -> None:
    audio_codec = audio_codec.lower()
    audio_mux_engine = audio_mux_engine.lower()
    if audio_codec not in {"ia4m", "pc16"}:
        raise SystemExit(f"unsupported audio codec: {audio_codec}")
    if audio_mux_engine not in {"python", "native"}:
        raise SystemExit(f"unsupported audio mux engine: {audio_mux_engine}")
    if audio_mux_engine == "native" and audio_codec != "ia4m":
        raise SystemExit("--audio-mux-engine native currently supports --audio-codec ia4m only")
    ...
    with video_mivf.open("rb") as vf, out_path.open("wb") as out_file:
        ...
        audio_proc = start_ffmpeg_audio_pipe(audio_src, rate, channels, audio_stream)
        native_ia4m_proc = None
        if audio_codec == "ia4m" and audio_mux_engine == "native":
            native_ia4m_proc = start_native_ia4m_stream(samples_per_frame, 0)
            print("AUDIO MUX: using native IA4M packet encoder")
        ...
        try:
            while True:
                ...
                if audio_codec == "ia4m":
                    if native_ia4m_proc is not None:
                        abody = read_native_ia4m_body(native_ia4m_proc, pcm[:samples_per_frame * 2], frame_no)
                    else:
            ...
            if native_ia4m_proc is not None:
                if native_ia4m_proc.stdin:
                    native_ia4m_proc.stdin.close()
                native_stderr = native_ia4m_proc.stderr.read().decode("utf-8", errors="replace") if native_ia4m_proc.stderr else ""
                native_rc = native_ia4m_proc.wait()
                if native_rc != 0:
                    if native_stderr:
                        sys.stderr.write(native_stderr)
                    raise SystemExit(f"native IA4M helper failed (exit code {native_rc})")
...
def deploy_output(output_path: Path) -> None:
    ...
    for edition in range(4):
        sidecar_srt = make_subtitle_sidecar_path(output_path, edition)
        if sidecar_srt.exists():
            shutil.copy2(sidecar_srt, sd_card / sidecar_srt.name)
...
def encode_one(
    ...
    try:
        ...
        print("============================================================")
        mux_t0 = time.time()
        mux_audio_into_mivf(temp_video_only, input_path, output_path, settings.audio_rate, settings.audio_channels, workdir, settings.audio_codec, settings.audio_offset_ms, settings.audio_mux_engine, settings.audio_stream)
        mux_elapsed = time.time() - mux_t0
        e1["audio_mux"]=mux_elapsed
        e0_stage(workdir,e0_state,"mux_complete",e1,mux_output=str(output_path))

        subtitle_elapsed = 0.0
        if settings.subtitle_stream is not None:
            print()
            print("============================================================")
            print("Extracting Subtitle Edition")
            print("============================================================")
            subtitle_t0 = time.time()
            subtitle_sidecar = extract_subtitle_sidecar(input_path, output_path, settings.subtitle_stream, settings.subtitle_edition)
            subtitle_elapsed = time.time() - subtitle_t0
            print(f"AUTHORED SUBTITLE SIDECAR: {subtitle_sidecar} (edition {settings.subtitle_edition}, player sidecar only -- not embedded in the MIVF)")
            e1["subtitle_extract"]=subtitle_elapsed
            e0_stage(workdir,e0_state,"subtitle_complete",e1,subtitle_sidecar=str(subtitle_sidecar))
        ...
        if settings.subtitle_stream is not None:
            print(f"     subtitle extract:  {fmt_time(subtitle_elapsed)} ({subtitle_elapsed:.1f}s)")
        if make_m2y2:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--video-stream", type=int, default=0, help="0-based video stream among video streams; default 0 preserves legacy first-video behavior")
    parser.add_argument("--audio-stream", type=int, default=0, help="0-based audio stream among audio streams; default 0 preserves legacy first-audio behavior")
    parser.add_argument("--subtitle-stream", type=int, default=None, help="0-based subtitle stream among subtitle streams; omit to author no subtitle sidecar (legacy behavior, unchanged)")
    parser.add_argument("--subtitle-edition", type=int, default=0, help="player sidecar slot 0-3 (source/main.c's subtitle_track_index convention); only meaningful with --subtitle-stream, default 0")
    ...
    parser.add_argument("--audio-mux-engine", choices=["python", "native"], default="python", help="IA4M mux engine: python is the trusted reference; native uses tools/mivf_ia4m_stream.exe for faster IA4M packet encoding")
...
def main() -> None:
    ...
    settings = EncodeSettings(
        ...
        audio_mux_engine=args.audio_mux_engine,
        video_stream=args.video_stream,
        audio_stream=args.audio_stream,
        subtitle_stream=args.subtitle_stream,
        subtitle_edition=args.subtitle_edition,
        keyint=args.keyint,
        qp=args.qp,
    ...
    print(f"  source streams:  video={settings.video_stream} audio={settings.audio_stream} (0-based by type)")
    print(f"  audio:           {settings.audio_codec.upper()} {settings.audio_rate}Hz ch={settings.audio_channels}")
    print(f"  audio_mux:       {settings.audio_mux_engine}")
