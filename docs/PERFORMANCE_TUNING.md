...
- **Frame rate:** 30 fps at 400×240 is a common sweet spot; exact rational rates like
  ...
  [encoding.md](encoding.md#frame-rate) for the audio-rate compatibility rules that come
  with rational rates.
...
- The DVD-style menu caches full rendered frames and only redraws on an actual change
  ...
  than every loop iteration — see [player-ui.md](player-ui.md#cached-frame-rendering).
...
Benchmark data and methodology: [benchmark-data/mivf_benchmark_data_audit.md](benchmark-data/mivf_benchmark_data_audit.md).
...
[docs/validation-status.md](validation-status.md) for exactly what has and hasn't been
