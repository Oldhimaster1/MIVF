...
3. Each chunk is submitted to a `ThreadPoolExecutor` task that calls the native helper
   ...
   `hierarchical` — see [PERFORMANCE_TUNING.md](PERFORMANCE_TUNING.md#motion-search-modes)).
...
[PERFORMANCE_TUNING.md](PERFORMANCE_TUNING.md#motion-search-modes) for the measured
numbers.
...
requested frame rate and audio rate — see [encoding.md](encoding.md#frame-rate) for which
...
- Reduce `--chunk-frames` when memory is constrained (also affects throughput — see the
  ...
  [PERFORMANCE_TUNING.md](PERFORMANCE_TUNING.md#e2-benchmarking-jobs-and-chunk-frames)).
