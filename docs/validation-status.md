...
| **Hardware-verified** | Confirmed working on a physical New/Old 3DS. |
...
| MoFlex (`.moflex`) playback | **Hardware-verified** (as of the last committed release) | See [MOFLEX_STATUS.md](MOFLEX_STATUS.md). Unaffected by the current uncommitted diff. |
...
| Resume/bookmark modal + pixel progress bar | **Build-verified** | B and START both mean "start over" in this modal — see [CONTROLS.md](CONTROLS.md). |
...
| Motion search: `diamond`, `fast`, `hybrid` | **Experimental** | Byte-identical-to-baseline determinism confirmed; quality/size tradeoffs measured on a small local clip set — see [PERFORMANCE_TUNING.md](PERFORMANCE_TUNING.md). |
...
confirmation notwithstanding. Physical-hardware regression testing of the current UI
work remains outstanding regardless of emulator status.
