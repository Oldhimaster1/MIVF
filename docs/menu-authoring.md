...
`mivf_menu_load_background`) — see [player-ui.md](player-ui.md) for how these are used
at runtime and [FILES_AND_SIDECARS.md](FILES_AND_SIDECARS.md) for the full sidecar
reference.
...
The image is an Azahar showcase capture proving that a custom `.screensaver.cover` can
be loaded and rendered. It is not a test of the normal idle trigger or long-running
bounce behavior.
