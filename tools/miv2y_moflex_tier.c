...
/* MIVF_PROFILE_STAGES_V1: diagnostic-only stage timing, off by default.
   Build with -DMIVF_PROFILE_STAGES to accumulate wall-clock time spent in
   (a) the per-block motion-search dispatch (find_best_mv_64* family) and
   (b) write_page() I/O, printed as one extra summary line at the end of
   main(). Never affects encode output, mode decisions, or control flow --
   this is pure measurement, added specifically to answer "where does the
   time go now that SAD is SSE2 and global-MV capping exists" without
   guessing. clock() is used (not QueryPerformanceCounter) for portability;
   accumulated over thousands of per-block calls this resolution is more
   than adequate to separate stages that differ by whole seconds. */
#ifdef MIVF_PROFILE_STAGES
#include <time.h>
static clock_t g_prof_motion_search_ticks = 0;
static clock_t g_prof_write_page_ticks = 0;
static long long g_prof_skip_shortcut_hits = 0;
static long long g_prof_blocks_total = 0;
#endif

/* MIVF_E3_ZERO_DIST_SKIP_SHORTCUT_V1 (off by default -- gated so a direct
   A/B hash comparison against the unmodified path can be built and run,
   not just reasoned about):

   PROOF OF EXACTNESS (not a heuristic): RDO_CONSIDER's cost for every
   candidate is visual_dist + lambda*rate_bytes, where visual_dist =
   rdo_nssd_64(...) is always >= 0 (it is (sum_of_squared_diffs + 4) >> 3,
   which can only be 0 when every one of the 64 per-pixel diffs is 0) and
   rate_bytes is always >= 1 for every RDO_CONSIDER call this function
   ever makes (the macro's own `if ((token_len) > 0)` guard means any
   candidate that would have rate/token_len == 0, e.g. QRESZ when
   qz_len==0, is excluded from consideration entirely rather than being
   evaluated at rate 0). SKIP is rate_bytes=1 and is always the FIRST
   candidate evaluated. So if SKIP's own distortion is exactly 0 (the
   current 8x8 block is byte-identical to the co-located block in the
   previous frame -- checked here via rdo_sad_64, which is 0 if and only
   if rdo_nssd_64 would also be 0, since both are non-negative aggregates
   of the same per-pixel diffs and both are 0 exactly when every diff is
   0), SKIP's cost is exactly lambda*1. For any later candidate to beat
   it, RDO_CONSIDER requires strictly candidate_cost < lambda, i.e.
   visual_dist < lambda*(1 - rate_bytes) <= 0 for every rate_bytes >= 1 --
   impossible since visual_dist >= 0. This holds for every mode this
   function evaluates after SKIP (DELTA, QRES, QRESZ, TRANSFORM,
   TRANSFORMZ, GMVCOPY_INTERNAL at the same rate=1, MVCOPY/MVCOPYP,
   MVQRES, MVQRESZ, MVTRANSFORM(Z)(P), SOLID) -- none can ever produce a
   strictly lower cost than an already-zero-distortion SKIP, so skipping
   their evaluation entirely for such a block cannot change best_kind,
   best_recon, or any byte of the output. This also means motion search
   itself does not need to run for such a block, since its only purpose
   here is feeding the MV-based candidates this shortcut has already
   proven cannot win. RAW is unaffected (keyframe-only; SKIP is only
   evaluated when !keyframe && prev, so the two paths never overlap). */
#ifdef MIVF_E3_SKIP_SHORTCUT
#define MIVF_E3_SKIP_SHORTCUT_ENABLED 1
#else
#define MIVF_E3_SKIP_SHORTCUT_ENABLED 0
#endif
...
#if MIVF_E3_SKIP_SHORTCUT_ENABLED
                if (sad_skip == 0) {
                    /* See MIVF_E3_ZERO_DIST_SKIP_SHORTCUT_V1 above: no
                       remaining candidate can ever beat a zero-distortion
                       SKIP, so nothing past this point can change the
                       outcome for this block. */
#ifdef MIVF_PROFILE_STAGES
                    g_prof_skip_shortcut_hits++;
#endif
                    goto mivf_e3_block_rdo_done;
                }
#endif
...
#ifdef MIVF_PROFILE_STAGES
                    g_prof_motion_search_ticks += clock() - __prof_ms_t0;
#endif
...
#ifdef MIVF_MEASURE_PSNR
    /*
     * PERF1A_OPTIONAL_PSNR_V1
     *
     * Closed-loop reconstruction PSNR is validation-only measurement.
     * It never participates in mode selection, RDO, reconstruction,
     * packet generation, timing, or container output.
     *
     * Production builds intentionally omit the full-frame squared-error
     * pass. Validation builds restore the historical measurement with:
     *
     *   -DMIVF_MEASURE_PSNR
     */
...
#ifdef MIVF_PROFILE_STAGES
        g_prof_write_page_ticks += clock() - __prof_wp_t0;
#endif

        free(pkt);

#ifdef MIVF_MEASURE_PSNR
        /* Validation-only closed-loop reconstruction-error measurement. */
...
#ifdef MIVF_PROFILE_STAGES
    {
        clock_t total_ticks = clock() - __prof_main_t0;
        double total_s = (double)total_ticks / (double)CLOCKS_PER_SEC;
        double ms_s = (double)g_prof_motion_search_ticks / (double)CLOCKS_PER_SEC;
        double wp_s = (double)g_prof_write_page_ticks / (double)CLOCKS_PER_SEC;
        double other_s = total_s - ms_s - wp_s;

        printf(
            "PROF_STAGES total=%.3fs motion_search=%.3fs (%.1f%%) "
            "write_page=%.3fs (%.1f%%) other_rdo_and_setup=%.3fs (%.1f%%) "
            "e3_skip_shortcut_hits=%lld blocks_total=%lld hit_rate=%.1f%%\n",
            total_s,
            ms_s, total_s > 0.0 ? 100.0 * ms_s / total_s : 0.0,
            wp_s, total_s > 0.0 ? 100.0 * wp_s / total_s : 0.0,
            other_s, total_s > 0.0 ? 100.0 * other_s / total_s : 0.0,
            g_prof_skip_shortcut_hits, g_prof_blocks_total,
            g_prof_blocks_total > 0
                ? 100.0 * (double)g_prof_skip_shortcut_hits / (double)g_prof_blocks_total
                : 0.0
        );
    }
#endif
